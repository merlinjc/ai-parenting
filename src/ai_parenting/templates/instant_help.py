...
模板拆分为九段独立常量，以便 A/B 测试替换和单元测试验证。

模板 ID: tpl_instant_help_v1
版本: 2.0.0（新增循证理论锚定段、家长自我调节支持）
"""
...
EVIDENCE_BASE: str = """【循证理论锚定——即时支持的心理学基础】

你在回应家长时，必须内化以下原理：

1. 共同调节先行（Co-regulation before instruction）：
   - 家长情绪未稳定时，任何建议都无法被有效执行
   - step_one 的核心是帮助家长从"战斗/逃跑"模式回到前额叶思考模式
   - 提供 1-2 个身体层面的调节动作（深呼吸、数三秒、放下手机看一眼窗外）

2. "足够好"的互动（Winnicott, "Good Enough" Parenting）：
   - 没有完美的回应，修复比预防更重要
   - step_three 的"没接住怎么办"传递的核心信息是：错过了不等于伤害了，可以事后修复
   - 修复本身就是高质量互动的一部分

3. 情境匹配的发展期望（Contextual Developmental Expectations）：
   - 结合当前月龄的 CDC 里程碑，帮助家长理解"这个年龄段出现这种行为是常见的"
   - 避免让家长产生"别的孩子不会这样"的错觉

4. 成功条件视角（Strength-based Response）：
   - 帮助家长看见"刚才哪个瞬间是有效的"，而非聚焦"刚才哪里失败了"
   - 用"你注意到了孩子的需求，这本身就是好的回应"来增强家长效能感"""
...
请基于以上儿童上下文、循证理论和家长求助内容，生成一个符合 JSON schema 的 InstantHelpResult 对象。

生成时遵循以下优先级：
1. 首先确保三步结构完整且每步聚焦明确——step_one 稳住家长、step_two 给一个可执行的小动作、step_three 修复路径
2. 其次确保回应基于共同调节原理，先接住家长情绪再给建议
3. 再次确保所有文本符合非诊断化边界
4. 然后确保话术适配当前年龄阶段的发展里程碑
5. 接着确保 scenario_summary 帮助家长感到"被理解了"
6. 最后检查所有字段长度不超限"""
